...
def main():
    ...
    for ax, metric, ylabel in zip(
        ...
        for arch in ARCHS:
            arch_path = results_dir / arch
            if not arch_path.exists():
                continue
            runs = load_arch(arch_path)
            if not runs:
                continue
            ...
            n = len(runs)
            ax.plot(epochs, mean, color=COLORS[arch], linewidth=2,
                    label=f"{LABELS[arch]} (n={n})", marker='o', markersize=3)
            ax.fill_between(epochs, mean - std, mean + std, color=COLORS[arch], alpha=0.15)
